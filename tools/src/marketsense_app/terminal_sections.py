"""Focused terminal report sections.

Each function owns one output concern. The public print_terminal entry point
stays in terminal.py so CLI imports and output ordering remain compatible.
"""
...
def print_scan_header(summary: dict[str, Any]) -> None:
...
def print_pricing_statuses(
    summary: dict[str, Any], rows: list[dict[str, Any]], top: int,
) -> None:
    weapon_rows = [
        row for row in rows
...
def print_diagnostic_sections(
    summary: dict[str, Any], rows: list[dict[str, Any]],
    confidence_threshold: float, chunk_size: int,
    low_confidence_chunk: int, availability_chunk: int,
    heuristic_gap_chunk: int,
) -> None:
