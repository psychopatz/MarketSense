...
TOOL_RECIPE_MARKER = "__MARKETSENSE_TOOL_RECIPE_LITERAL__"


def render_bridge(
    spec_literal: str, sandbox_literal: str = "{}", yield_literal: str = "{}",
    tool_recipe_literal: str = "{}",
) -> str:
    ...
    if TOOL_RECIPE_MARKER not in template:
        raise RuntimeError(f"Bridge template is missing {TOOL_RECIPE_MARKER}")
    return template.replace(SPEC_MARKER, spec_literal, 1).replace(
        SANDBOX_MARKER, sandbox_literal, 1
    ).replace(YIELD_MARKER, yield_literal, 1).replace(
        TOOL_RECIPE_MARKER, tool_recipe_literal, 1
    )
